...
public class CustomExecutor extends ThreadPoolExecutor {
    /**
     ...
     The maximum priority of tasks submitted to this executor.
     */
    private int maxPrio;
    /**
    ...
    public <T> Future<T> submit(Task<T> task) {
        maxPrio = Math.min(maxPrio, task.getPrio());
        DesignAdapter<T> adpt = new DesignAdapter<>(task, maxPrio);
        super.execute(adpt);
        return adpt;
    }
    /**
    ...
    protected void beforeExecute(Thread t, Runnable r) {
        DesignAdapter DA = (DesignAdapter) r;
        maxPrio = Math.max(DA.getPriority(), maxPrio);
        }
